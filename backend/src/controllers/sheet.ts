...
export async function getSheet(req: Request, res: Response) {
  ...
  if(!req.session.user.isAuth && sheetData.login_required) {
    ...
    res.render("pages/sheet", makeRenderObject({ title: `${sheetName}`, sheetName: sheetName, sheetPath: sheetPath }, req));
  }
}
...
export async function getSheetEditHistory(req: Request, res: Response) {
  ...
  if(!req.session.user.isAuth && sheetData.login_required) {
    ...
    res.render("pages/edit_history", makeRenderObject({ title: `${sheetName}`, sheetName: sheetName, editHistoryPath: editHistoryPath }, req));
  }
}
