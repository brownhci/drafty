...
export async function findUserByField(fieldName: string, fieldValue: string | number, callback: findUserByFieldCallbackType) {
  ...
  try {
    ...
    db.logDatabaseError(error, "error during finding user", "warn");
    callback(error);
  }
}
...
export async function createUser(user: Partial<UserRow>, callback: Function) {
  try {
    ...
    db.logDatabaseError(error, "error during creating user", "warn");
    callback(error);
  }
}
...
export async function updateUser(updates: Partial<UserRow>, constraints: Partial<UserRow>, callback: Function) {
  try {
    ...
    db.logDatabaseError(error, "error during updating existing user", "warn");
    callback(error);
  }
}
...
export async function deleteUser(constraints: Partial<UserRow>, callback: Function) {
  try {
    const [results, fields] = await db.query("DELETE FROM ?? WHERE ? LIMIT 1", [userTableName, constraints]);
    callback(null, results, fields);
  } catch (error) {
    db.logDatabaseError(error, "error during deleting user", "warn");
    callback(error);
  }
}