...
export class FuseSelect {
  ...
  private createOptionContainerFromFuseResult(fuseResult: Array<Fuse.FuseResult<Option>>) {
    ...
    for (let i = 0; i < fuseResult.length; i++) {
      ...
      this.createOptionElement(option, matches);
    }
   ...
   */
  private createOptionElement(option: Option, matches: ReadonlyArray<Fuse.FuseResultMatch> = []) {
    //if(true) {
      ...
      this.optionContainer.appendChild(optionElement);
    //} 
  }
