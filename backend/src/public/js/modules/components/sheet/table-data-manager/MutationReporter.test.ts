...
describe("Attribute Change", () => {
  ...
  test("Attribute change and catch event", done => {
    ...
    const reporter = new MutationReporter((correctedMutations, mutations, observer, _reporter) => {
      _reporter.report(correctedMutations);
    });

    reporter.observe(element, MutationReporter.createMutationObserverInit(true, false, true, ["class"]));
...
describe("Character Change", () => {
  ...
  test("Append a text node", done => {
    const reporter = new MutationReporter((correctedMutations, mutations, observer, _reporter) => {
      ...
      expect(correctedMutations[0].type).toBe("characterData");
      expect(correctedMutations[0].target.textContent).toBe("foo");
      expect(correctedMutations[0].oldValue).toBeFalsy();
...
describe("observing and unobserving", () => {
  ...
  test("reobserve", done => {
    ...
    reporter.reobserveToExecute(() => target.replaceChild(document.createTextNode("ignored"), target.firstChild), target);
